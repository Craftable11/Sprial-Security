...
public class ItemUtil {
    ...
    public static String toString(ItemStack itemStack) {
        ...
        stringBuilder.append(String.format(
                "itemID: %d, metaData: %d, stackSize: %d, ", itemStack.itemID,
                itemStack.getItemDamage(), itemStack.stackSize));

        if (itemStack.hasTagCompound()) {
            stringBuilder.append(String.format("nbtTagCompound: %s, ",
                    itemStack.getTagCompound().toString()));
        }

        stringBuilder.append(String.format("itemName: %s, className: %s ",
    ...
    public static boolean compare(ItemStack first, ItemStack second) {
        ...
        if (first != null && second != null) {
            ...
            if (first.itemID == second.itemID) {
                ...
                if (first.getItemDamage() == OreDictionary.WILDCARD_VALUE
                ...
                if (first.getItemDamage() == second.getItemDamage()) {
                    ...
                    if (first.stackSize == second.stackSize) {
                        // If at least one of the ItemStacks has a
                        // NBTTagCompound, test for equality
                        if (first.hasTagCompound() || second.hasTagCompound()) {

                            // If one of the stacks has a tag compound, but not
                            // both, they are not equal
                            if (!(first.hasTagCompound() && second
                                    .hasTagCompound()))
                                return false;
                            else
                                return first.getTagCompound().equals(
                                        second.getTagCompound());
                        } else
                            return true;
                    }
                }
    ...
    public static ArrayList<CustomWrappedStack> collateStacks(
            List<CustomWrappedStack> unCollatedStacks) {
        ...
        for (int i = 0; i < unCollatedStacks.size(); i++) {
            ...
            if (collatedStacks.size() == 0) {
                ...
                for (int j = 0; j < collatedStacks.size(); j++) {
                    if (unCollatedStacks.get(i).getWrappedStack() instanceof ItemStack
                            && collatedStacks.get(j).getWrappedStack() instanceof ItemStack) {
                        if (compare((ItemStack) unCollatedStacks.get(i)
                                .getWrappedStack(), (ItemStack) collatedStacks
                                .get(j).getWrappedStack())) {
                            collatedStacks.get(j).setStackSize(
                                    collatedStacks.get(j).getStackSize() + 1);
                            found = true;
                        }
                    } else if (unCollatedStacks.get(i).getWrappedStack() instanceof OreStack
                            && collatedStacks.get(j).getWrappedStack() instanceof OreStack) {
                        if (OreStack.compareStacks((OreStack) unCollatedStacks
                                .get(i).getWrappedStack(),
                                (OreStack) collatedStacks.get(j)
                                        .getWrappedStack())) {
                            collatedStacks.get(j).setStackSize(
                                    collatedStacks.get(j).getStackSize() + 1);
                            found = true;
    ...
    public static boolean hasColor(ItemStack itemStack) {
        ...
        return !itemStack.hasTagCompound() ? false : !itemStack
                .getTagCompound().hasKey(Strings.NBT_ITEM_DISPLAY) ? false
                : itemStack.getTagCompound()
                        .getCompoundTag(Strings.NBT_ITEM_DISPLAY)
                        .hasKey(Strings.NBT_ITEM_COLOR);
    }
    ...
    public static int getColor(ItemStack itemStack) {
        ...
        if (nbtTagCompound == null)
            ...
            return displayTagCompound == null ? Integer.parseInt(
                    ...
                    .hasKey(Strings.NBT_ITEM_COLOR) ? displayTagCompound
                    .getInteger(Strings.NBT_ITEM_COLOR) : Integer.parseInt(
                    Colours.PURE_WHITE, 16);
        }
    }
    ...
    public static void setColor(ItemStack itemStack, int color) {
        ...
        if (itemStack != null) {
            ...
            if (!nbtTagCompound.hasKey(Strings.NBT_ITEM_DISPLAY)) {
                nbtTagCompound.setCompoundTag(Strings.NBT_ITEM_DISPLAY,
                        colourTagCompound);
            }
