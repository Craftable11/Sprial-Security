        ...
        for (Object recipe : CraftingManager.getInstance().getRecipeList()) {
            ...
            if (recipe instanceof IRecipe) {
                ...
                if (((IRecipe) recipe).getRecipeOutput() instanceof ItemStack) {
                    ...
                    CustomWrappedStack recipeOutput = new CustomWrappedStack(
                            ((IRecipe) recipe).getRecipeOutput());
                    ArrayList<CustomWrappedStack> recipeInputs = RecipeHelper
                            .getRecipeInputs((IRecipe) recipe);
                    ...
                    if (recipeOutput.getWrappedStack() instanceof ItemStack) {
                        ...
                        if (itemStack.getItemDamage() == OreDictionary.WILDCARD_VALUE
                                && !wildCards.contains(recipeOutput)) {
                    ...
                    for (CustomWrappedStack inputStack : recipeInputs) {
                        ...
                        if (inputStack.getWrappedStack() instanceof ItemStack) {
                            ...
                            itemStack = (ItemStack) inputStack
                                    .getWrappedStack();

                            if (itemStack.getItemDamage() == OreDictionary.WILDCARD_VALUE
                                    && !wildCards.contains(inputStack)) {
        ...
        if (recipe instanceof ShapedRecipes) {
            ...
            for (int i = 0; i < shapedOreRecipe.getInput().length; i++) {
                ...
                if (shapedOreRecipe.getInput()[i] instanceof ArrayList) {
                    CustomWrappedStack oreStack = new CustomWrappedStack(
                            shapedOreRecipe.getInput()[i]);

                    if (oreStack.getWrappedStack() instanceof OreStack) {
                        recipeInputs.add(new CustomWrappedStack(shapedOreRecipe
                                .getInput()[i]));
                    }
