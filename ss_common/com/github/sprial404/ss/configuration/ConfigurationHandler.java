...
public class ConfigurationHandler {
    ...
    public static final String CATEGORY_BLOCK_PROPERTIES = Configuration.CATEGORY_BLOCK
            + Configuration.CATEGORY_SPLITTER + "properties";
    ...
    public static void init(File configFile) {
        ...
        try {
            ...
            ConfigurationSettings.LAST_DISCOVERED_VERSION = configuration.get(
                    ...
                    .getString();
            ConfigurationSettings.LAST_DISCOVERED_VERSION_TYPE = configuration
                    .get(CATEGORY_GENERAL,
            ...
            /* Block configs */
            BlockIds.GEM_BLOCK = configuration.getBlock(Strings.GEM_BLOCK_NAME,
                    BlockIds.GEM_BLOCK_DEFAULT).getInt(
                    BlockIds.GEM_BLOCK_DEFAULT);
            BlockIds.GEM_ORE = configuration.getBlock(Strings.GEM_ORE_NAME,
                    BlockIds.GEM_ORE_DEFAULT).getInt(BlockIds.GEM_ORE_DEFAULT);
            BlockIds.MACHINE_STRUCTURE = configuration.getBlock(
                    Strings.MACHINE_STRUCTURE_NAME,
                    BlockIds.MACHINE_STRUCTURE_DEFAULT).getInt(
                    BlockIds.MACHINE_STRUCTURE_DEFAULT);

            /* Block property configs */
            configuration.addCustomCategoryComment(CATEGORY_BLOCK_PROPERTIES,
                    "Custom block properties");

            /* Item configs */
            ItemIds.GEM = configuration.getItem(Strings.GEM_NAME,
                    ItemIds.GEM_DEFAULT).getInt(ItemIds.GEM_DEFAULT);
            ItemIds.BATTERY = configuration.getItem(Strings.BATTERY_NAME,
                    ItemIds.BATTERY_DEFAULT).getInt(ItemIds.BATTERY_DEFAULT);

            /* KeyBinding configs */
            configuration
                    .addCustomCategoryComment(
                            CATEGORY_KEYBIND,
                            "Keybindings for Sprial Security. See http://www.minecraftwiki.net/wiki/Key_codes for mapping of key codes to keyboard keys");
        } catch (Exception e) {
